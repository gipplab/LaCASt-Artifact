...
public class MLPBlueprintTree {
    ...
    public static final Pattern PATTERN_DOTS = Pattern.compile("\\\\[lc]?dots[cbmio]?");
    public static final Pattern PATTERN_DOTS_COMMA = Pattern.compile("([^,\\s])\\s*\\\\dots");
    public static final Pattern PATTERN_FRAC = Pattern.compile("\\\\[it]?frac");
    public static final Pattern PATTERN_NEQ = Pattern.compile("\\\\ne[^\\w]");
    ...
    public static String preCleaning(String constraint){
        ...
        constraint = PATTERN_DOTS.matcher(constraint).replaceAll("\\\\dots");
        constraint = PATTERN_DOTS_COMMA.matcher(constraint).replaceAll("$1, \\\\dots");
        constraint = PATTERN_FRAC.matcher(constraint).replaceAll("\\\\frac");
        constraint = PATTERN_NEQ.matcher(constraint).replaceAll("\\\\neq");
        constraint = PATTERN_SPACES.matcher(constraint).replaceAll("");
        return constraint;
    }
}

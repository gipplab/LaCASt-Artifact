...
//                "]";
    ...
    private NumericCalculation getNumericCalculation(Expr result) throws MathLinkException {
        ...
        String s = mathematicaInterface.evaluate("ToString[FullForm["+singleResultArgs[0].toString()+"]]");
        if ( !s.isBlank() ) s = s.substring(1, s.length()-1);
        ...
        for ( Expr varValPair : varValPairs.args() ) {
            ...
            String key = mathematicaInterface.evaluate("ToString[FullForm["+varValPairArr[0].toString()+"]]");
            String val = mathematicaInterface.evaluate("ToString[FullForm["+varValPairArr[1].toString()+"]]");

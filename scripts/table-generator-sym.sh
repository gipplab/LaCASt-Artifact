...
BASEPATH="./dlmf/results-original"
...
for FILE in $FILES; do
  ...
  read -r -a arrMath <<< $(gawk 'match($0, /.*STARTED_TEST_CASES: ([0-9]+),.*ERROR_TRANS: ([0-9]+),.*MISSING: ([0-9]+),.*SUCCESS_TRANS: ([0-9]+),.*SUCCESS_SYMB: ([0-9]+),.*SUCCESS_UNDER_EXTRA_CONDITION: ([0-9]+).*,.*FAILURE: ([0-9]+).*/, arry) {print (arry[5]+arry[6])","arry[4]","arry[7]","arry[1]","arry[3]","arry[2]}' $FILE)
  ...
  read -r -a arrMaple <<< $(gawk 'match($0, /.*STARTED_TEST_CASES: ([0-9]+),.*ERROR_TRANS: ([0-9]+),.*MISSING: ([0-9]+),.*SUCCESS_TRANS: ([0-9]+),.*SUCCESS_SYMB: ([0-9]+),.*SUCCESS_UNDER_EXTRA_CONDITION: ([0-9]+).*,.*FAILURE: ([0-9]+).*/, arry) {print (arry[5]+arry[6])","arry[4]","arry[7]","arry[1]","arry[3]","arry[2]}' $mapleF)
...
#  err=$(bc <<< "${arrMath[3]} - ${arrMath[1]} - ${arrMath[4]}")
  ...
  SUMERR=$((SUMERR + arrMath[5]))

  printf "\\\verb|%s| & %2d & %3d & %3d & (%4.1f\\%%) & %3d & (%4.1f\\%%) & %3d & (%4.1f\\%%) & %3d & %3d & %2d \\\\\\ \\hline\n" "${id}" "${num#0}" "${arrMath[3]}" "${oldTrans}" "${avgOT}" "${arrMaple[1]}" "${avgMAP}" "${arrMath[1]}" "${avgMath}" "${arrMaple[4]}" "${arrMath[4]}" "${arrMath[5]}"
done
